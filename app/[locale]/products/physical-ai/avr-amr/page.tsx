...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: isKo ? "AVR/AMR 자율이동로봇" : "AVR/AMR Autonomous Mobile Robot",
    description: isKo
...
export default async function AVRAMRPage({ params }: PageProps) {
  const { locale } = await params;
  const isKo = locale === "ko";
        ...
        <div className="relative max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-base text-[#669DFD] font-semibold mb-4">
              {isKo ? "AVR/AMR — 자율이동로봇 플랫폼" : "AVR/AMR — Autonomous Mobile Robot Platform"}
            </p>
