...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: isKo ? "RX75 - 비전형 로봇암" : "RX75 Vision Robot Arm",
    description: isKo
      ? "AI 비전과 7축 휴머노이드 로봇암이 결합된 스마트 자동화 솔루션."
      : "A smart automation solution combining AI vision with a 7-axis humanoid robot arm.",
  };
}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-gray-50 border-b border-gray-100">
            ...
            {models.map((m, i) => (
              <th key={m} className={`text-center px-5 py-3.5 text-xs font-bold whitespace-nowrap ${i === 3 ? "text-[#669DFD]" : "text-gray-500"}`}>
                {m}
              </th>
            ))}
          </tr>
        ...
        <tbody className="divide-y divide-gray-50">
          {rows.map((row, i) => (
            <tr key={row.label} className={i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}>
              ...
              {row.values.map((v, j) => (
                <td key={j} className={`px-5 py-3 text-center whitespace-nowrap ${j === 3 ? "text-gray-900 font-semibold bg-[#669DFD]/5" : "text-gray-700"}`}>
                  {v}
                </td>
              ))}
            </tr>
...
export default async function Rx75VisionPage({ params }: PageProps) {
  ...
  const specs = [
    { label: isKo ? "모델명" : "Model", value: "RX75 - 비전형" },
    { label: isKo ? "자유도" : "DOF", value: "7" },
    { label: isKo ? "작업반경" : "Working Radius", value: "732mm" },
    { label: isKo ? "무게" : "Weight", value: "8.4kg" },
    ...
    { label: isKo ? "엔드 이펙터 카메라" : "End-Effector Camera", value: "Intel RealSense D405" },
  ...
  const features = isKo
    ? ["7축 자유도", "5kg Payload", "내장형 6축 힘·토크 센서", "Intel RealSense D405"]
    : ["7-Axis DOF", "5kg Payload", "Built-in 6-Axis Force/Torque Sensor", "Intel RealSense D405"];
        ...
        <div className="relative max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h1 className="text-5xl md:text-6xl font-extrabold text-white mb-3 tracking-tight">
              RX75 <span className="text-white/40">{isKo ? "비전형" : "Vision"}</span>
            </h1>
          ...
          <div className="relative h-64 md:h-80">
            <Image
              src="/products/humanoid/robot-arm/rx75-vision.jpeg"
              alt="RX75 Vision"
              fill
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <p className="text-sm text-gray-400 mt-6">
            ...
            {", "}
            <Link href={`/${locale}/products/humanoid/robot-arm/rx75s`} className="text-[#669DFD] font-semibold hover:underline">RX75S</Link>
            {isKo ? ", " : ", and "}
