...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: isKo ? "RX71 - 표준형 로봇암" : "RX71 Standard Robot Arm",
    description: isKo
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-gray-50 border-b border-gray-100">
            ...
            {models.map((m, i) => (
              <th key={m} className={`text-center px-5 py-3.5 text-xs font-bold whitespace-nowrap ${i === 0 ? "text-[#669DFD]" : "text-gray-500"}`}>
                {m}
              </th>
            ))}
          </tr>
        ...
        <tbody className="divide-y divide-gray-50">
          {rows.map((row, i) => (
            <tr key={row.label} className={i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}>
              ...
              {row.values.map((v, j) => (
                <td key={j} className={`px-5 py-3 text-center whitespace-nowrap ${j === 0 ? "text-gray-900 font-semibold bg-[#669DFD]/5" : "text-gray-700"}`}>
                  {v}
                </td>
              ))}
            </tr>
...
export default async function Rx71Page({ params }: PageProps) {
  ...
  const specs = [
    { label: isKo ? "모델명" : "Model", value: "RX71 - 표준형" },
    { label: isKo ? "자유도" : "DOF", value: "7" },
    { label: isKo ? "작업반경" : "Working Radius", value: "474mm" },
    { label: isKo ? "무게" : "Weight", value: "3.8kg" },
    { label: isKo ? "반복 위치 측정 정밀도" : "Repeatability", value: "±0.2mm" },
    { label: isKo ? "가반 하중" : "Payload", value: "3kg" },
    { label: isKo ? "TCP 선속도" : "TCP Linear Speed", value: "≤ 1.8 m/s" },
    { label: isKo ? "정격 소비 전력" : "Rated Power", value: "≤ 100W" },
    { label: isKo ? "최대 출력" : "Max Power", value: "≤ 250W" },
    { label: isKo ? "공급 전압" : "Power Supply", value: "DC 24V" },
    { label: isKo ? "엔드 이펙터 카메라" : "End-Effector Camera", value: isKo ? "옵션" : "Optional" },
  ...
  const features = isKo
    ? ["7축 자유도", "3kg Payload", "내장형 6축 힘·토크 센서"]
    : ["7-Axis DOF", "3kg Payload", "Built-in 6-Axis Force/Torque Sensor"];
