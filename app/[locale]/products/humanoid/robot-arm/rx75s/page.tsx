...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: isKo ? "RX75S - 표준형 로봇암" : "RX75S Standard Robot Arm",
    description: isKo
      ? "좁은 공간에서도 강력한 성능을 구현하는 컴팩트 7축 휴머노이드 로봇암."
      : "A compact 7-axis humanoid robot arm delivering strong performance even in tight spaces.",
  };
}
...
export default async function Rx75sPage({ params }: PageProps) {
  const { locale } = await params;
  const isKo = locale === "ko";

  const specs = [
    { label: isKo ? "모델명" : "Model", value: "RX75S - 표준형" },
    { label: isKo ? "자유도" : "DOF", value: "7" },
    { label: isKo ? "작업반경" : "Working Radius", value: "691mm" },
    ...
    { label: isKo ? "엔드 이펙터 카메라" : "End-Effector Camera", value: isKo ? "옵션 (GMSL)" : "Optional (GMSL)" },
